 ...
 * Fixed:
 *  - exports.makeWASocket di dalam IIFE async (race condition dengan main.js)
 *  - conn.getFile duplikat — definisi kedua overwrite
 *  - conn.cMod duplikat sync/async — overwrite
 *  - axios tidak di-import → conn.getBuffer selalu crash
 *  - Assignment ke const id di chats.set event
 *  - send3ButtonLoc — parameter button3/row3 hilang dari signature
 *  - convert & trueFileName implicit global (tanpa let/const)
 *  - opts['legacy'] tanpa global.
 *  - this.sendFile di arrow function conn.reply → undefined
 *  - parseMention — push lid bukan resolved PN
 *  - await fs.writeFileSync (sync tidak perlu await)
 *  - conn.sendGroupV4Invite menggunakan this. di luar class
 *  - smsg diubah async karena getJid async
 */
...
// [FIX] Tambahkan axios yang hilang — dipakai oleh conn.getBuffer
...
exports.makeWASocket = (connectionOptions, options = {}) => {
    // [FIX] Pakai global.opts bukan opts (variabel lokal main.js)
    ...
    conn.getJid = async (sender) => {
        ...
        for (const chat of Object.values(conn.chats || {})) {
            ...
            const participant = chat.metadata.participants.find(p =>
                p.lid === jid || p.id === jid || p.jid === jid ||
                (p.phoneNumber && jid === conn.decodeJid(p.phoneNumber))
            )
            if (participant) {
                let resolved = participant.phoneNumber || participant.jid || participant.id ||
    ...
    conn.ev.on('chats.set', async ({ chats }) => {
        for (const { id: rawId, name, readOnly } of chats) {
            // [FIX] Assignment ke const id dalam destructuring → gunakan let decodedId
            let decodedId = conn.decodeJid(rawId)
            if (!decodedId) continue
            const isGroup = decodedId.endsWith('@g.us')
            let chat = conn.chats[decodedId]
            if (!chat) chat = conn.chats[decodedId] = { id: decodedId }
            chat.isChats = !readOnly
            if (name) chat[isGroup ? 'subject' : 'name'] = name
            if (isGroup) {
                const metadata = await conn.groupMetadata(decodedId).catch(_ => null)
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        ...
        conn.chats[id] = { ...conn.chats[id], subject: groupMetadata.subject, metadata: groupMetadata }
    })
    ...
    // [FIX] conn.getFile hanya didefinisikan SEKALI — versi duplikat dihapus
    ...
    conn.filter = (text) => {
        ...
        if (/[aiueo][aiueo]([qwrtypsdfghjklzxcvbnm])?$/i.test(text)) return text.substring(text.length - 1)
            ...
            for (let huruf of mati) {
                if (text.endsWith(huruf)) resu = res[res.length - 2]
            }
    ...
    conn.rand = async (isi) => isi[Math.floor(Math.random() * isi.length)]

    conn.resize = async (buffer, uk1, uk2) => {
        return new Promise(async (resolve, reject) => {
            let baper = await jimp.read(buffer)
            let ab = await baper.resize(uk1, uk2).getBufferAsync(jimp.MIME_JPEG)
            resolve(ab)
        })
    }
    ...
    conn.sendMedia = async (jid, path, quoted, options = {}) => {
        ...
        // [FIX] messageType & pase harus pakai let/const bukan implicit global
    ...
    conn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        if (/webp/.test(type.mime))  mtype = 'sticker'
            ...
            // [FIX] convert pakai let — bukan implicit global
            let convert = await (ptt ? toPTT : toAudio)(file, type.ext)
            file     = convert.data
            // [FIX #3] pathFile dari converter sudah dihapus setelah dibaca ke buffer
            // Jangan kirim { url: pathFile } — file tidak exist lagi di disk
            // Gunakan buffer langsung sebagai payload audio
            mtype    = 'audio'
        ...
        return await conn.sendMessage(jid, {
            ...options, caption, ptt,
            // [FIX #3] Audio kirim buffer (file), tipe lain tetap { url: pathFile }
            [mtype]: mtype === 'audio' ? file : { url: pathFile },
            mimetype
        }, { ...opt, ...options })
    }
    ...
    conn.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        // [FIX] Guard jika writeExifImg return undefined (metadata kosong)
        if (!buffer) throw new Error('[sendImageAsSticker] writeExifImg return undefined — packname/author kosong')
    ...
    conn.sendVideoAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        if (!buffer) throw new Error('[sendVideoAsSticker] writeExifVid return undefined — packname/author kosong')
        await conn.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
...
`.trim()
    ...
    conn.reply = (jid, text = '', quoted, options) => {
        // [FIX] this.sendFile di arrow function → undefined. Ganti ke conn.sendFile
        return Buffer.isBuffer(text)
            ? conn.sendFile(jid, text, 'file', '', quoted, false, options)
            : conn.sendMessage(jid, { ...options, text, mentions: conn.parseMention(text) }, { quoted, ...options, mentions: conn.parseMention(text) })
    }
    ...
    conn.sendGroupV4Invite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', options = {}) => {
        let msg = proto.Message.fromObject({
            groupInviteMessage: proto.GroupInviteMessage.fromObject({
                ...
                groupJid: jid,
                // [FIX] this.getName tidak ada di arrow/async function — ganti conn.getName
                groupName: groupName ? groupName : await conn.getName(jid),
                caption
            })
        })
        // [FIX] this.prepareMessageFromContent & this.relayWAMessage → conn.*
        let message = await conn.prepareMessageFromContent(participant, msg, options)
    ...
    conn.sendBut = async (jid, content, footer, button1, row1, quoted) => {
        ...
        return await conn.sendMessage(jid, {
            text: content, footer, buttons, headerType: 1,
    ...
    conn.send2But = async (jid, content, footer, button1, row1, button2, row2, quoted) => {
        ...
        return await conn.sendMessage(jid, { text: content, footer, buttons, headerType: 1 }, { quoted })
    }
    ...
    conn.sendButtonImg = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
        ...
        const buttonMessage = {
            image: file, fileLength: 887890909999999,
            caption: contentText, footer: footerText,
            mentions: await conn.parseMention(contentText + footerText),
            ...options, buttons, headerType: 4
        }
    ...
    conn.sendH3Button = async (jid, content, displayText, link1, displayCall, number, quickReplyText, id, quickReplyText2, id2, quickReplyText3, id3, quoted) => {
        let template = generateWAMessageFromContent(jid, proto.Message.fromObject({
            templateMessage: {
                hydratedTemplate: {
                    hydratedContentText: content,
                    hydratedButtons: [
                        { urlButton:       { displayText, url: link1 } },
    ...
    // [FIX] conn.cMod hanya didefinisikan SEKALI — versi sync (duplikat pertama) dihapus
    // Dipertahankan versi async yang lebih lengkap
    ...
    // conn.cMods — varian sync, dipertahankan terpisah
    ...
    conn.sendHButtonLoc = async (jid, buffer, content, footer, distek, link1, quick1, id1, quoted) => {
        ...
        }), { userJid: conn.user.jid, quoted, mentions: conn.parseMention(content + footer) })
        return await conn.relayMessage(jid, template.message, { messageId: template.key.id })
    }
    ...
    conn.sendButtonLoc = async (jid, buffer, content, footer, button1, row1, quoted, options = {}) => {
        ...
        let buttonMessage = { location: { jpegThumbnail: buffer }, caption: content, footer, buttons, headerType: 6 }
    ...
    // [FIX] Tambahkan button3/row3 ke signature yang sebelumnya hilang
    ...
    conn.sendButtonVid = async (jid, buffer, contentText, footerText, button1, id1, quoted, options) => {
        ...
        const buttonMessage = {
            video: file, fileLength: 887890909999999,
            caption: contentText, footer: footerText,
    ...
    conn.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        ...
        // [FIX] trueFileName pakai let — bukan implicit global
        let trueFileName = attachExtension ? (filename + '.' + type.ext) : filename
        // [FIX] Ganti await fs.writeFileSync → fs.promises.writeFile (sync tidak perlu await)
        await fs.promises.writeFile(trueFileName, buffer)
        return trueFileName
    }

    // [FIX] parseMention — jika LID ada di cache, push resolved PN (jid), bukan lid
    conn.parseMention = (text) => {
        ...
        for (const id of match) {
            const lid = `${id}@lid`
            const jid = conn.isLid.get(lid)
            // [FIX] Jika ada di cache, push JID hasil resolve (bukan lid mentah)
            if (conn.isLid.has(lid) && jid) out.push(jid)
    ...
    conn.getName = async (jid = '', withoutContact = false) => {
        ...
        if (areJidsSameUser(jid, conn.user.jid)) return conn.user.name || conn.user.verifiedName || 'Me'
    ...
    conn.processMessageStubType = async (m) => {
        ...
        // [FIX] chat didefinisikan di sini lalu dipakai di emitGroupUpdate lewat closure — sudah benar
        ...
        for (const message of m) {
            try {
                ...
                if (isGroup) {
                    ...
                    if (sender !== chat) {
                        let sChats = conn.chats[sender]
                        if (!sChats) sChats = conn.chats[sender] = { id: sender }
                        if (!sChats.name) sChats.name = message.pushName || sChats.name || ''
                    }
                } else if (!chats.name) {
    ...
    conn.format = (...args) => util.format(...args)

    conn.getBuffer = async (url, options) => {
        try {
            // [FIX] axios sudah di-import di atas — tidak akan crash lagi
    ...
    // [FIX #2] conn.serializeM harus async — smsg sudah diubah jadi async
    // Versi sync sebelumnya mengembalikan Promise bukan object pesan
    conn.serializeM = async (m) => await exports.smsg(conn, m)
...
exports.smsg = async (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        if (chatJid?.endsWith('@lid')) {
            chatJid = (await conn.getJid(chatJid)) || chatJid
        }
        m.chat = conn.decodeJid(
            chatJid || m.message?.senderKeyDistributionMessage?.groupId || m.key.remoteJid || ''
        )
        m.isGroup = m.chat?.endsWith('@g.us') ||
            m.key.remoteJid?.endsWith('@g.us') ||
            m.message?.senderKeyDistributionMessage?.groupId?.endsWith('@g.us') || false
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {
                ...
                m.quoted.reply       = (text, chatId, options) => conn.reply(chatId || m.chat, text, vM, options)
                m.quoted.copy        = async () => await exports.smsg(conn, M.create(M.toObject(vM)))
                m.quoted.forward     = (jid, forceForward = false) => conn.copyNForward(jid, vM, forceForward)
    ...
    // [FIX #4] m.forward tidak pass options → ReferenceError. Tambahkan options = {} ke signature
...
// [FIX #1] Tambahkan .catch() pada IIFE agar error tidak ditelan diam-diam
// Tanpa ini, jika IIFE gagal (misal Baileys tidak terinstall), exports.makeWASocket
// tidak pernah ter-assign dan main.js hang selama 15 detik lalu throw timeout
})().catch(err => {
    console.error('[FATAL] simple.js IIFE gagal dijalankan:', err)
    process.exit(1)
})
