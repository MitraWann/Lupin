...
function smartTruncate(text, limit = 1800) {
  if (text.length <= limit) return text
  const sections = text.split(/\n{2,}/)
  const perSection = Math.floor(limit / sections.length)
  return sections.map(s => s.slice(0, Math.max(perSection, 200))).join('\n\n').slice(0, limit) + '\n...[teks dipotong]'
}
...
async function extractDocumentText(buffer, mimetype = '', filename = '') {
  if (mimetype.includes('pdf')) {
    ...
    const data = { text: smartTruncate(result.pages.map(p => p.text).join('\n')) }
  ...
  if (PLAIN_TEXT_EXTS.test(filename) || mimetype.startsWith('text/')) {
    return smartTruncate(buffer.toString('utf8').trim())
  }
...
module.exports = { getAuth, parseStream, toDataUrl, askAichatting, extractDocumentText, MODELS }
